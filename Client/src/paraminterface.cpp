...
bool ParamInterface::setData(const QModelIndex &index, const QVariant &value,int role){
    ...
    ZSS::ZPM::_()->changeParam(item->parentItem()->data(0),item->data(0),value);
...
QVariant ParamInterface::data(const QModelIndex &index, int role) const {
    if (index.isValid() && role >= KeyRole) {
        TreeItem *item = static_cast<TreeItem*>(index.internalPointer());
//        qDebug() << "data " << role - KeyRole << index << "      " << item->data(role - KeyRole);
...
void ParamInterface::setupModelData(){
    ...
    foreach(QString groupName, ZSS::ZPM::_()->allGroups()){
        QStringList value;
        value << groupName << " " << " ";
        auto* groupParent = new TreeItem(value,rootItem);
        rootItem->appendChild(groupParent);
        foreach(QString key,ZSS::ZPM::_()->allKeys(groupName)){
            auto&& temp = ZSS::ZPM::_()->value(groupName,key).toString();
            QStringList value;
